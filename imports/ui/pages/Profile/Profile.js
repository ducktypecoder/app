...
class Profile extends React.Component {
        ...
        {Object.keys(user.services).map(service =>
            (<div key={service} className={`LoggedInWith ${service}`}>
              <div className="ServiceIcon">
                <i
                  className={`fa fa-${service === 'facebook'
                    ? 'facebook-official'
                    : service}`}
                />
              </div>
  ...
  renderToken(loading, user) {
    if (loading) return <div />;

    return (
      <div>
        <h4 className="page-header">Your Token</h4>
        <form>
          <FormGroup>
            <input
              type="text"
        ...
        <p>
          <em>
            Copy & paste this token in your local project to connect with your
            online ducktypecoder account.
          </em>
        </p>
      </div>
    );
